...
| [Node.js](node.qmd) | `shell: node` + `prompt: ">"` |
| [Ruby](ruby.qmd) | `shell: irb` + `prompt-regex` |
| [R](r.qmd) | `shell: R` + `prompt: ">"` |
| [SQLite](sqlite.qmd) | `shell: sqlite3` + `prompt: "sqlite>"` |
| [DuckDB](duckdb.qmd) | `shell: duckdb` + `prompt: "D"` |
...
| Python | Built-in via PyREPL (3.13+) — use `disable-auto-indent: true` |
| Node.js | Built-in (colored output values) |
| Ruby | Built-in via reline (Ruby 3.x+) — use `disable-auto-indent: true` |
| DuckDB | Built-in (colored SQL and tables) |
| Bash / R / SQLite / sh | Not available |
