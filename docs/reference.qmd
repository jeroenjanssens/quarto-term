...
|--------|:-------:|:--------:|:----:|:----:|---------|-------------|
| `assert` | | | | x | (none) | Regex/substring pattern to validate in line output |
| `cache` | x | x | | | `true` | Cache results to skip re-execution on unchanged inputs |
| `disable-auto-indent` | x | x | | | `false` | Counteract REPL auto-indentation on continuation lines (for Python PyREPL, Ruby irb) |
| `callouts` | | | x | | `[]` | Line specs to annotate with callout markers |
...
|--------|---------|-------------|
| `auto-indent` | `false` | Strip REPL auto-indentation from continuation lines (for Python PyREPL) |
...
| `callouts` | `[]` | Line specs for callout annotations |
...
The `remove`, `truncate`, and `callouts` options accept a list of **line specs** that identify lines in the rendered output (input + output combined). Line numbers refer to the original output before any removals or truncations.
